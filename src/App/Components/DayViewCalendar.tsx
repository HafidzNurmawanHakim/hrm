...
import { DayObj, ItemHour, TaskItem, ToRenderItem } from "../Library/_types/ScheduleTypes";
...
const DayViewCalendar: FC<DayViewProps> = ({ currentMonthForDay, load }) => {
   const task = useSelector((state: RootState) => state.ScheduleSlice.taskInHour);
   ...
   function generateEl() {
      ...
      taskHour.forEach((itemHour, rowIndex) => {
         if (element.length === 0) {
            ...
            let findEl = Jsx.filter((el, index) => el.id && !el.id.startsWith("empty"));
