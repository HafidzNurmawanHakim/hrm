...
const CustomTable: FC<TableProviderProps> = ({
  columns,
  data,
  title,
  enableSelect = false,
  ...
  const renderCell = useCallback(
    (item: Record<string, any>, columnKey: Key) => {
            ...
            <Dropdown>
              ...
              >
                {statusOptions.map((status) => (
                  <DropdownItem
                    key={status.uid}
                    className="capitalize dark:text-light"
                  >
  ...
  const classNames = useMemo(
    () => ({
      wrapper: ["bg-foreground shadow-none dark:bg-background dark:text-light"],
      th: ["text-default-500"],
      // td: [
